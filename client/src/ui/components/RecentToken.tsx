...
import { getLongDate } from "../../util/date";
...
const RecentToken: React.FC<Props> = ({ token }) => {//creates UI element with the most recent token, formatted with CSS/Tailwind
  return (
    <div
      ...
      className="recentToken relative z-10 bg-gradient-to-br from-white to-white w-full h-80 bg-opacity-0 shadow-custom rounded group transition transform duration-100 hover:scale-105 cursor-pointer"
    >
      <div className="h-60 grid grid-rows-custom my-10 mx-10">
        <div className="date flex items-center">
          ...
          <h4 className="font-Nunito font-bold text-projectCyan-light ml-3.5 inline-block group-hover:text-white duration-150 2xl:text-base xl:text-sm sm:text-xs">{getLongDate(token.createdTimestamp)}</h4>
        </div>
